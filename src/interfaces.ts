export interface TargetUrl404Errors {
  ...
  export interface InternallyLinkedUrlsInSitemaps {
    url: string;           // The internally linked URL
    sitemapUrl: string;    // Corresponding URL in the sitemap
    match: boolean;        // Whether the URL matches or not
  }
  ...
  export interface ImportantPagesBlockedByRobotsTxt {
    url: string;           // The URL blocked by robots.txt
    reason: string;        // Reason why the URL is considered important
  }

  export interface UnnecessaryPagesBlockedByRobotsTxt {
    url: string;           // The URL blocked by robots.txt
    reason: string;        // Reason why the page is considered unnecessary
  }
  ...
  export interface BlogUpdateFrequency {
    ...
    updateFrequency: string; // Update frequency, e.g., "Monthly", "Weekly"
  }
  ...
  export interface AuditData {
    ...
    xmlSitemap?: XMLSitemap[]; // Does the site have an XML sitemap?
    unnecessaryPagesInSitemap?: UnnecessaryPagesInSitemap[]; // The sitemap contains unnecessary pages
